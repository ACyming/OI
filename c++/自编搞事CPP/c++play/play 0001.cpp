...
void Cout(int i, int j, int a) {
	...
	if (a != 4) {
		...
		if ((m[i][j] == 1 || m[i][j] == 8) && m[i - 1][j] >= 10 && m[i - 1][j] <= 30 && a != 5) {
			int M = m[i][j];    //炸弹下落
			m[i][j] = m[i - 1][j], m[i - 1][j] = M;
			if (M == 1) SetPos(i - K - 1, j), cout << "  ";
		}
		if (m[i][j] == 1 && m[i - 1][j] == 9 && a != 5) {
			m[i][j] = 9, m[i - 1][j] = 1, SetPos(i - K - 1, j), Color(0), cout << "  ";    //宝石下落
			jk = 1;
		}
		if (m[i][j] >= 10 && m[i][j] <= 30) {
			m[i][j]++;    //炸弹计时
			if (m[i][j] % 3 == 0) Color(4);
			else Color(5);
			SetPos(i - K, j), cout << "●";
		}
		if (m[i][j] == 1 && m[i - 1][j] == 7 && a != 5) {
			m[i][j] = 7, m[i - 1][j] = 1, SetPos(i - K - 1, j), Color(0), cout << "  ";    //岩浆下落
			jk = 1;
		}
		if (m[i][j] == 0 && m[i - 1][j] == 7 && R <= 2) m[i][j] = m[i - 1][j], m[i - 1][j] = 1, SetPos(i - K - 1, j), cout << "  "; //岩浆融解
		if (m[i][j] == 7 && R > 2 && R <= 5) {
			int r = rand() % 3 - 1;    //岩浆扩散
			if (m[i][j + r] == 1) m[i][j + r] = m[i][j];
			if (m[i][j + r] == 8) m[i][j + r] = 2;
		}
		...
		if (m[i][j] == 1 && m[i - 1][j] == 8 && a != 5) {
			m[i][j] = 8, m[i - 1][j] = 1, SetPos(i - K - 1, j), Color(0), cout << "  ";    //水下落
			jk = 1;
		}
		...
		if (m[i][j] == 8 && R > 7 && R <= 9) {
			int r = rand() % 3 - 1;    //水扩散
			if (m[i][j + r] == 1) m[i][j + r] = m[i][j];
			if (m[i][j + r] == 7) m[i][j + r] = 2;
		}
		if (m[i][j] == 31) SetPos(i - K, j), cout << "  ", Zha(i, j);
	} else if (a != 5) { //界外
		if (m[i][j] >= 3 && m[i][j] < 6) m[i][j]++;
		if (m[i][j] == 6) m[i][j] = 1;
...
void TrigleBomb(int i, int j, int r, int mr) {
	...
	Zb++;
	B[Zb].xx = i + 1, B[Zb].yy = j, B[Zb].Zhong = 5, B[Zb].rr = r + 1, B[Zb].mr = mr;
}
void GunBomb(int i, int j, int r, int mr) {
	if (mr <= 0) return;
	Zb++;
	B[Zb].xx = i, B[Zb].yy = j + 2, B[Zb].Zhong = 6, B[Zb].rr = r, B[Zb].mr = mr - 1;
	Zb++;
...
void GunBomb2(int i, int j, int r, int mr) {
	if (mr <= 0) return;
	Zb++;
	B[Zb].xx = i, B[Zb].yy = j - 2, B[Zb].Zhong = 7, B[Zb].rr = r, B[Zb].mr = mr - 1;
	Zb++;
	B[Zb].xx = i, B[Zb].yy = j, B[Zb].Zhong = 1, B[Zb].rr = r - 1, B[Zb].mr = r;
}
void ThreeBomb(int x, int y, int s, int ms) {
	for (int i = 1; i <= 3; i++) {
		int Xx = rand() % (2 * s + 1) - s, Yy = rand() % (2 * s + 1) - s;
		Zb++;
...
void Jiao() {
	SetPos(0, 2);
	Color(1), cout << "●", Color(7), cout << "   ■ ", Color(8), cout << "   ■ ", Color(4), cout << "   ●    ", Color(9), cout << "~ ", Color(0), cout << "   ", Color(10), cout << "~ ", Color(3), cout << "   ◆";
	SetPos(2, 2);
...
void Start() {
	...
	cout << "             ■  ■    ■     ";
	SetPos(12, 3);
	Color(8), cout << "  ■■■ ", Color(5), cout << "    ■■■  ■    ■ ";
	SetPos(13, 3);
	Color(8), cout << "  ■", Color(5), cout << "矿", Color(8), cout << "■ ", Color(5), cout << "    ■  ■ ■■■■■";
	SetPos(14, 3);
	Color(8), cout << "  ■■■ ", Color(5), cout << "    ■  ■           ";
	SetPos(15, 3);
	cout << "             ■■■  ■■■■ ";
...
St:
	...
	while (Win == 0) {
		...
		if (kbhit()) {
			char e = _getch();
			if (e == ' ' && m[X - 1][Y] != 0 && m[X - 1][Y] != 2 && (m[X + 1][Y] <= 10 || m[X + 1][Y] >= 31) && Kb == 0) m[X][Y] = 10, X--;
		}
		int zb = Zb;
