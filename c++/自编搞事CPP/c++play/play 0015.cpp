...
void Color(int a) {
	...
	if (a == -8) SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), BACKGROUND_RED);
	if (a == 1) SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_INTENSITY | FOREGROUND_GREEN | FOREGROUND_BLUE);
	if (a == -11) SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), BACKGROUND_INTENSITY | BACKGROUND_GREEN | BACKGROUND_BLUE);
	if (a == 2) SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_INTENSITY | FOREGROUND_GREEN);
	if (a == 3) SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_INTENSITY | FOREGROUND_RED | FOREGROUND_BLUE);
	if (a == 4) SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_INTENSITY | FOREGROUND_RED);
	if (a == -5) SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), BACKGROUND_INTENSITY | BACKGROUND_RED | BACKGROUND_GREEN);
...
struct bullet {
	float x, y, vx, vy, fx1, fx2, fx3;
	int w, t, k, l, ex1, ex2, ex3; //what,time,kill,life;
...
void Map(int a) {
	if (a == 1) {
		for (register int i = 1; i <= 50; i++)for (register int j = 1; j <= 100; j++) {
				if (n[i][j] != m[i][j]) {
					C(i, j);
				}
			}
...
//Map1：更新地图
void N(float x, float y, int w) {
	int xx = (int)x, yy = (int)y; //放置点在地图上
...
void Circle(int x, int y, int r, int num, int start, int whatnumber, int source) {
	if (r < 0) return;
	if (r == 0) num = 1;
	int next = 360 / num;
	for (register int i = 0; i < num; i++) {
		...
		B[br].t = 0;
		B[br].l = 1;
		B[br].x = x + Sin(i * next + start) * r;
		B[br].y = y + 2 * Cos(i * next + start) * r;
	}
}
void Rock_Circle(float x0, float y0, float xr, float ybig, int staytime, int source, int num) {
	if (xr < 0) return;
	for (register int i = x0 - xr; i <= x0 + xr; i++) {
		float Ay = ybig * 2 * sqrt(xr * xr - (i - x0) * (i - x0));
		for (register float j = y0 - Ay; j < y0 + Ay; j++) {
			br++;
			B[br].w = 3;
			B[br].staytime = staytime;
			B[br].t = 0;
			B[br].l = 1;
			B[br].x = i;
			B[br].y = to(j);
...
void Rock_line(float x0, float y0, float x, float y, int source) {
	...
	bool yy = 0;
	if (Ax < 0) swap(x, x0), swap(y, y0), Ax *= -1, Ay *= -1;
	if (Ay < 0) yy = 1;
	...
	while (i <= x && ((j <= y && yy == 0) || (j >= y && yy == 1))) {
		...
		B[br].staytime = to(R);
		B[br].source = source;
		B[br].num = B[source].num;
		if (source == -2) B[br].source = 0, B[br].num = -2;
		i += ax;
		j += ay;
	}
}
void Circle_bomb(int x, int y, int number, int start, float speed, int source, int num) {
	int next = 360 / number;
	for (register int i = 0; i < number; i++) {
		...
		B[br].source = source;
		B[br].num = num;
		B[br].vx = Sin(i * next + start) * speed;
		B[br].x = x - B[br].vx;
		B[br].vy = 2 * Cos(i * next + start) * speed;
		B[br].y = y - B[br].vy;
	}
}
...
void Put_Round_Bullet(int ex, float v) { //1:反伤
	for (register int i = 0; i <= v * 4; i += 5) {
		br++;
		B[br].w = 5;
		B[br].t = 0;
		B[br].l = 1;
...
void Level_Bullet() {
	if (T <= 50 && T % 5 == 0) Rand_put_Bullet();
	else if (T <= 100) return;
	else if (T <= 150 && T % 5 == 0) Rand_put_Knife(T % 2);
	else if (T <= 200) return;
	else if (T <= 250 && T % 7 == 0) Rand_sin_Bullet();
	else if (T <= 300) return;
	else if (T <= 350 && T % 7 == 0) Circle_bomb(20, 40, 15, T % 15, 2, 0, 1);
	else if (T <= 400) return;
	else if (T <= 450 && T % 7 == 0) Rand_back_Bullet();
	else if (T <= 500) return;
	else if (T <= 550 && T % 10 == 0) Rand_brouce_Bullet();
	else if (T <= 600) return;
	else if (T <= 1000) {
		if (T % 5 == 0) Rand_put_Bullet();
		if (T % 21 == 0) Rand_put_Knife(T % 2);
		if (T % 39 == 0) Rand_sin_Bullet();
		if (T % 90 == 0) Circle_bomb(20, 40, 15, T % 15, 2, 0, 1);
		if (T % 18 == 0) Rand_back_Bullet();
		if (T % 34 == 0) Rand_brouce_Bullet();
	} else if (T <= 2000) {
		if (T % 6 == 0) Rand_put_Bullet();
		if (T % 17 == 0) Rand_put_Knife(T % 2);
		if (T % 23 == 0) Rand_sin_Bullet();
		if (T % 60 == 0) Circle_bomb(20, 40, 15, T % 15, 2, 0, 1);
		if (T % 9 == 0) Rand_back_Bullet();
		if (T % 28 == 0) Rand_brouce_Bullet();
	} else {
		if (T % 10 == 0) Rand_put_Bullet();
		if (T % 13 == 0) Rand_put_Knife(T % 2);
		if (T % 9 == 0) Rand_sin_Bullet();
		if (T % 40 == 0) Circle_bomb(20, 40, 15, T % 15, 2, 0, 1);
		if (T % 8 == 0) Rand_back_Bullet();
		if (T % 27 == 0) Rand_brouce_Bullet();
	}
}
void Move_bullet() {
	for (register int i = bl; i <= br; i++) {
		if (B[bl].l == 0) bl++;
		if (B[i].l == 1 && TimeStop == 0) B[i].t++;
		else continue;
		bool del;
		...
		//    if(level==1){
		if (Sleep_1 == 0 && s[to(x)][to(y)] >= 1 && s[to(x)][to(y)] <= 2 && w == 3 && ex1 != 1 && num >= 0 && num <= 1) {
			Delete_B(source), del = 1, Delete_B(i);
			EXP += 0.3;
			if (people != 4) Sleep_1 = 1;
			if (s[to(x)][to(y)] == 1) {
				B[i].w = 2;
				B[i].t = 0;
				B[i].l = 1;
		...
		if (w == 2) {
			if (x > 40 || y > 80 || x < 1 || y < 1 || (abs(vx) <= 0.1 && abs(vy) <= 0.1) || (people == 4 && t >= 6 && num < 0)) {
				del = 1, Delete_B(i);
				continue;
			} else {
				Rock_line(x, y, x + vx, y + vy, i);
		...
		if (w == 3) {
			...
			if (x > 40 || y > 80 || x < 1 || y < 1 || t >= staytime || (ex2 == 1 && m[to(x)][to(y)] > 0 && Sleep_2 == 0)) {
				del = 1, Delete_B(i), N(x, y, 0);
				if (ex1 == 1 || num == -2) s[to(x)][to(y)] = 0;
				continue;
			}
			if (num != 0) N(x, y, num);
			else N(x, y, 1);
			if (ex1 == 1) s[to(x)][to(y)] = 1;
			else if (num == -2) s[to(x)][to(y)] = 2;
		}//残影
		if (w == 4) { //Hurt
			Circle(x, y, 1 + t * v, 90, 1, num, i);
			if (t >= staytime) del = 1, Delete_B(i);
		}
		if (w == 5) { //Round
			if (angle > maxangle || angle < minangle) {
				del = 1, Delete_B(i);
			...
			for (register int a = angle; a < angle + v; a += 3) {
				Rock_Circle((int)(X + Sin(a)*xr), (int)(Y + 2 * Cos(a)*xr), 1, 0.5, staytime, i, -2);
			}
			if (TimeStop == 0) angle += v;
		}
		if (w == 6) { //Sin
			if (x > 40 || y > 80 || x < 1 || y < 1) {
				del = 1, Delete_B(i);
				continue;
			}
			for (register int a = angle; a < angle + v; a += 3) {
				Rock_Circle((int)(x + Sin(a)*xr), (int)y, 1, 0.5, staytime, i, 1);
			}
		...
		if (w == 9) { //Crab_Attack
			vx = rand() % 15 - 7, vy = rand() % 29 - 14;
			if (ex1 == 3) vx = rand() % 11 - 5, vy = rand() % 11 - 5;
			if (ex1 == 2) vx = rand() % 5 + 2;
			if (TimeStop == 0) {
				float xx = fmax(fmin(x + vx, 39), 1), yy = fmax(fmin(y + vy, 80), 1);
				Rock_line(x, y, xx, yy, -2);
				x = xx, y = yy;
				if (ex1 == 3) X = to(x), Y = to(y), unkillT = 2, Crab = 1;
			}
			if (x >= 39 || y >= 80 || x <= 1 || y <= 1) {
				del = 1, Delete_B(i);
				if (ex1 == 3) Crab = 2;
				continue;
			}
		...
		if (del == 0) { //事实上不用，有continue了
			B[i].x = x, B[i].y = y, B[i].vx = vx, B[i].vy = vy;
			B[i].fx1 = fx1, B[i].fx2 = fx2, B[i].fx3 = fx3;
			B[i].w = w, B[i].t = t, B[i].k = k;
			B[i].ex1 = ex1, B[i].ex2 = ex2, B[i].ex3 = ex3;
...
void Scissor() {
	...
	Svx = max(-Svmax, min(Svmax, Svx));
	Svy = max(-Svmax, min(Svmax, Svy));
...
void Start_level() {
	...
	while (Life == 1 && Lev < 10) {
		...
		if (an == 1) An++;
		if (an == 0 && An > 0) An = 0, Turnx = Turny = 0, an = 1;

		if (people == 1) {
			if (GetAsyncKeyState(67) & 0x8000) {
				if (Anc == 0) Anc = 1;
				Anc = min(9.9, Anc + 0.05);
			} else {
				if (Anc < 0) Anc += 0.02;
				if (Anc > -0.5 && Anc < 0.5) Anc = 0, Setpos(X - 2, Y / 2 - 2), printf("   ");
				if (Anc > 0) {
					int cy = Turny;
					if (Turnx == 0 && Turny == 0) cy = 3;
					put_Bullet(X, Y, D[0][Turnx] * (Anc + 2), D[1][cy] * (Anc + 2), -2);
					Anc = cMin;
				}
			}
			if (GetAsyncKeyState(86) & 0x8000) {
				if (Anv == 0) Anv = 1;
				Anv = min(9.9, Anv + 0.05);
			} else {
				if (Anv < 0) Anv += 0.02;
				if (Anv > -0.5 && Anv < 0.5) Anv = 0, Setpos(X - 2, Y / 2), printf("     ");
				if (Anv > 0) {
					Put_Round_Bullet(1, (int)(20 + Anv));
					Anv = vMin;
				}
			}
			if (GetAsyncKeyState(66) & 0x8000) {
				if (Anb == 0) Anb = 1;
				Anb = min(9.9, Anb + 0.05);
			} else {
				if (Anb < 0) Anb += 0.02;
				if (Anb > -0.5 && Anb < 0.5) Anb = 0, Setpos(X - 2, Y / 2 + 2), printf("     ");
				if (Anb > 0) {
					unkillT = 5;
					int nb = (int)(Anb * 3);
					Circle_bomb(X, Y, (nb + 3), rand() % (nb + 3), Anc * 0.1 + 2, 0, -2);
					Anb = bMin;
				}
			}
		}
		if (people == 2) {
			if (GetAsyncKeyState(87) & 0x8000) Stx = -1;
			else if (GetAsyncKeyState(83) & 0x8000) Stx = 1;
			else Stx = 0;
			if (GetAsyncKeyState(68) & 0x8000) Sty = 1;
			else if (GetAsyncKeyState(65) & 0x8000) Sty = -1;
			else Sty = 0;
		...
		if (people == 3) {
			...
			if (BeanBoo > 0) BeanBoo--;
			if (BeanBoo % 6 == 1) Put_brouce_Bullet(2, 0, 0, 0, 0, 0);
			if (Bean > 0) Bean--;
			if (Bean <= 0) Put_brouce_Bullet(1, 0, 0, 0, 0, 0), Bean = 20;
		}
		if (people == 4) {
			Sleep_3 = 0;
			if ((GetAsyncKeyState(67) & 0x8000) || (GetAsyncKeyState(86) & 0x8000)) {
				if (Anc == 0) Anc = 1;
				Anc = min(9.9, Anc + 0.05);
				if (Anc > 0 && Sleep_3 <= 2) {
					Sleep_3++;
					put_Bullet(X, Y, -3 + rand() % 60 / 10, -4 - rand() % 10 / 20, -4);
					Anc = 0;
					if (TimeStop == 0) Anc = cMin;
				}
			} else {
				if (Anc < 0) Anc += 0.02;
				if (Anc > -0.5 && Anc < 0.5) Anc = 0, Setpos(X - 2, Y / 2 - 2), printf("   ");
		...
		if (people == 5) {
			if (GetAsyncKeyState(67) & 0x8000) {
				...
				if (Anc > 0) {
					int cy = Turny;
					if (Turnx == 0 && Turny == 0) cy = 4;
					Thunder_step(X, Y, 3 * Anc + 4, -D[0][Turnx], -D[1][cy]);
					Anc = cMin;
				}
			}
			if (GetAsyncKeyState(86) & 0x8000) {
			...
			if (ThunderTime > 0) ThunderTime--;
			if (an == 0) ThunderTurn = 0;
		}
		if (people == 6) {
			if (GetAsyncKeyState(67) & 0x8000) {
				if (Anc == 0) Anc = 1;
		...
		if (an == 1 || kill == 1 || T <= 2) {
			if (Thunder != 0 || ThunderTime != 0) {
				Thunder_step(X, Y, 5, -D[0][Turnx], -D[1][Turny]);
			}
			for (register float i = X - 3; i <= X + 1; i++) for (register float j = Y - 8; j <= Y + 8; j++) C(i, j);
			if (Turnx == 1) X--;
			if (Turnx == 2) X++;
			...
			if (TimeStop > 0) Color(1 + rand() % 5);
			else if (unkillT > 0) Color(8);
			else Color(1);
			if (Turnx == 1 || (Turnx == 0 && Turny == 0)) {
				Setpos(X - 1, Y / 2.0), printf("o"), Setpos(X, Y / 2.0 - 0.5), printf("/+\\"), Setpos(X + 1, Y / 2.0);
				if (An % 2 == 0) printf("A");
				else printf("H");
			} else if (Turnx == 2) {
				Setpos(X - 1, Y / 2.0), printf("o"), Setpos(X, Y / 2.0 - 0.5), printf("\\+/"), Setpos(X + 1, Y / 2.0);
				...
				Setpos(X - 1, Y / 2.0 + 0.5), printf("o"), Setpos(X, Y / 2.0 - 0.5), printf("-+"), Setpos(X + 1, Y / 2.0 - 0.5);
				if (An % 2 == 0) printf("/>");
				else printf("v|");
			} else if (Turny == 4) {
				Setpos(X - 1, Y / 2.0 - 0.5), printf("o"), Setpos(X, Y / 2.0), printf("+-"), Setpos(X + 1, Y / 2.0);
				if (An % 2 == 0) printf("<\\");
				else printf("|v");
			}
		}
		...
		if (unkillT <= 0) for (register int i = 0; i <= 4; i++) if (n[(int)X + D[0][i]][(int)Y + D[1][i]] >= 1) {
					Blood -= Attack, kill = 1;
					Hurt(X, Y, 1);
					unkillT = unkillTmax;
				}
...
void Interface() {
	...
	Color(1), Setpos(11, 3), Slow("按 C 修改角色/查看规则，", 5);
	Color(2), Slow(" V 调整速度，", 5);
	Color(-5), Slow(" Y", 5);
	Color(5), Slow(" 开始游戏", 5);
...
void Time_check() {
	system("cls");
	Setpos(-1, 1);
	Slow("  现在开始速度调整，按 C 加速， V 减速， B 退出", 50);
	while (1) {
		if (kbhit()) {
			char g = _getch();
			if (g == 'c') speed *= 0.95;
			else if (g == 'v') speed *= 1.05;
			else if (g == 'b') break;
		}
		T++;
		if (T % 2 == 0) Rand_put_Bullet();
		Setpos(-1, 0), Color(0), printf("Speed: %0.3f   ", speed);
		Map(1);
		Move_bullet();
		Sleep(speed);
	}
	...
	while (1) {
		if (kbhit()) {
			...
			if (a == '1') {
				...
				Setpos(5, 1), Slow("角色编号：1", 10);
				people = 1;
				Setpos(7, 1), Slow("技能：", 10);
				Color(1), Setpos(9, 1), Slow("C ：遁之子弹（走路时反向发射，冷却时间：快） ", 10);
				skillC = 1;
				cMin = -1;
				Color(2), Setpos(11, 1), Slow("V ：护体神盾（有几率反弹子弹！冷却时间：中）", 10);
				skillV = 1;
				vMin = -5;
				Color(5), Setpos(13, 1), Slow("B ：元气烟花（无敌升级清场神器！冷却时间：慢）", 10);
				skillB = 1;
				bMin = -9;
				Setpos(15, 1), Slow("所有技能，按的时间越久，威力越强！", 10);
				break;
			} else if (a == '2') {
				system("cls");
				Setpos(1, 1), Slow("↑↓←→操作躲避子弹，活下去。", 10);
				Setpos(3, 1), Slow("角色名称：伍六七", 10);
				Setpos(5, 1), Slow("角色编号：2", 10);
				people = 2;
				Setpos(7, 1), Slow("技能：", 10);
				Color(1), Setpos(9, 1), Slow("AWSD （左手）操作剪刀！", 10);
				skillC = 0;
				Color(2), Setpos(11, 1), Slow("Z或X ：提升转速，加大范围！", 10);
				skillV = 0;
				Color(5), Setpos(13, 1), Slow("速度越快，最大范围越大！", 10);
				skillB = 0;
				break;
			} else if (a == '3') {
				system("cls");
				...
				Setpos(5, 1), Slow("角色编号：3", 10);
				people = 3;
				Setpos(7, 1), Slow("技能：", 10);
				Color(1), Setpos(9, 1), Slow("被动 ：发射无穿透性的豌豆", 10);
				skillC = 0;
				Color(2), Setpos(11, 1), Slow("V ：自动发球机（连续八方向发射豌豆！冷却时间：中）", 10);
				skillV = 1;
				vMin = -4;
				Color(5), Setpos(13, 1), Slow("B ：能量豆（你懂得，无敌升级清场神器！冷却时间：慢）", 10);
				skillB = 1;
				bMin = -6;
				break;
			} else if (a == '4') {
				...
				Setpos(5, 1), Slow("角色编号：4", 10);
				people = 4;
				Setpos(7, 1), Slow("技能：", 10);
				Color(1), Setpos(9, 1), Slow("C或V ：木大直拳（冷却时间：快，时停中极快）", 10);
				skillC = 1;
				cMin = -0.6;
				Color(5), Setpos(13, 1), Slow("B ：砸瓦鲁多（你懂得！冷却时间：慢）", 10);
				skillB = 1;
				bMin = -8;
				break;
			} else if (a == '5') {
				...
				Setpos(5, 1), Slow("角色编号：5", 10);
				people = 5;
				Setpos(7, 1), Slow("技能：", 10);
				Color(1), Setpos(9, 1), Slow("C ：雷霆一步（按得越久冲的越远！冷却时间：快）", 10);
				skillC = 1;
				cMin = -1;
				Color(2), Setpos(11, 1), Slow("V ：六连（获得六次瞬移机会！冷却时间：中）", 10);
				skillV = 1;
				vMin = -4;
				Color(5), Setpos(13, 1), Slow("B ：化雷（短时获得无限瞬移机会！冷却时间：慢）", 10);
				skillB = 1;
				bMin = -6;
				break;
			} else if (a == '6') {
				...
				Setpos(5, 1), Slow("角色编号：6", 10);
				people = 6;
				Setpos(7, 1), Slow("技能：", 10);
				Color(1), Setpos(9, 1), Slow("C ：概率波（用后就知道了hhh~！冷却时间：中）", 10);
				skillC = 1;
				cMin = -3;
				Color(2), Setpos(11, 1), Slow("V ：瞬闪柱（贯穿全场的攻击！冷却时间：中）", 10);
				skillV = 1;
				vMin = -5;
				Color(5), Setpos(13, 1), Slow("B ：波函动（化为波，短时获得无限瞬移机会！冷却时间：慢）", 10);
				skillB = 1;
				bMin = -8;
				Setpos(15, 1), Slow("一直想编一个攻击全看脸的角色呢hhh~", 10);
				break;
			}
		}
	}
