...
float X,Y,Vx,Vy,Ding,Blo=99564542,Hui,Bx1,By1,Bx2,By2,Bx3,By3,Bvx1,Bvy1,Bvx2,Bvy2,Bvx3,Bvy3,Bway[1001][2];
...
int main(){
    ...
    {
    	int x=1;
    	x=MessageBox(GetForegroundWindow(),"那就算了吧......","那就算了吧......",1);
    	
    	if(x==1)
    	{
    		MessageBox(GetForegroundWindow(),"那就跟你的电脑说再见吧！！","电脑将在30秒后关机！！",1);
    		system("shutdown -s -t 30");
		}
    	if(x==2)
		{
    		x=MessageBox(GetForegroundWindow(),"有点不舍？","",1);
    		if(x==1)
			{
				return 0;
			} 
			if(x==2)
			{
				x=MessageBox(GetForegroundWindow(),"再见！","",1);
				if (x==2)
				{
					char t;
					MessageBox(GetForegroundWindow(),"我让你静静！！","",1);
					cout<<"超好玩的对吧？\n"; 
					cout<<"给你一次机会！！\n输入我的名字";
					while(1)
					{
						int x=GetSystemMetrics(SM_CXSCREEN);//鼠标x轴 
 	  					...
 	  					if(t=='陈一鸣')
 	  					{
 	  						MessageBox(GetForegroundWindow(),"已关闭...","",1);
 	  						return 0;
						}
						else if(t=='SB'||t=='sb')
						{
							...
							if(t=='陈一鸣')
 	  						{
 	  							MessageBox(GetForegroundWindow(),"已关闭...算你识相！！","",1);
 	  							return 0;
							}
							else
							{
